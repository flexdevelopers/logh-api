class API::PicksController < API::BaseController
  ...
  def create
    ...
    if @pick.update_attributes(_pick_params)
      render json: { message: { type: SUCCESS, content: "You picked the #{@pick.squad.name} to lose in week #{week.number}. This pick will be hidden until the game starts." } }, status: :ok
    else
